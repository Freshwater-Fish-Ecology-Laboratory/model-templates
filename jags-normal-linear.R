### Simulate river velocity in relation to discharge
library(jmbr)
library(ggplot2)
...
velocity <- rnorm(100, mean = x, sd = bSigma)

data <- data.frame(Discharge = discharge,
                   Velocity = velocity)
ggplot(data = data, aes(x = Discharge, y = Velocity)) +
  geom_point()
...
model <- model("model {
  ...
  for(i in 1:nObs) {
    eVelocity[i] <- bIntercept + bDischarge * Discharge[i]  
    Velocity[i] ~ dnorm(eVelocity[i], bSigma^-2)
  }
}", new_expr = "
  for(i in 1:nObs) {
    eVelocity[i] <- bIntercept + bDischarge * Discharge[i]  
    prediction[i] <- eVelocity[i]
  }
", nthin = 10L, select_data = list(Discharge = c(0, 1000),
                                   Velocity = c(-100, 10000)))
...
gp <- ggplot(data = prediction, aes(x = Discharge, y = estimate)) +
  geom_point(data = data, aes(y = Velocity)) +
  geom_line() +
